...
- **Design:** parsing (turn text into structured data) is a separate step from applying
  (using that data to change what gets built) - kept `parse_prompt()` independent of
  `DeckBuilder` so interpretation and rendering don't get tangled together.
